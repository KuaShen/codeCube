...
#define btnW 12
#define titleWOfY 20
@interface JFChartView()<UIScrollViewDelegate>
{
...
-(UIView *)scrollBgView1{
    if (!_scrollBgView1) {
        _scrollBgView1 = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.chartScrollView.bounds.size.width-5, self.chartScrollView.bounds.size.height+20)];
...
//    gradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"0xf38b10" andAlpha:0.2].CGColor,(__bridge id)[UIColor colorWithHexString:@"0xf38b10" andAlpha:0.0].CGColor];
    gradientLayer.backgroundColor = COLOR_MAIN.CGColor;
    ...
    shapeLayer.strokeColor = COLOR_MAIN.CGColor;
...
-(void)addDataPointWith:(UIView *)view andArr:(NSArray *)leftData{
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        NSString *str = arr[i];
        CGSize textSize = [str boundingRectWithSize:CGSizeMake(200, 100) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10.0]} context:nil].size;;
        [detailLabel setFrame:CGRectMake((Xmargin)*i-textSize.width/2+btnW/2, height *(1 - tempHeight)-20, textSize.width+5, textSize.height+3)];
        detailLabel.text = str;
        detailLabel.textAlignment = NSTextAlignmentCenter;
        [detailLabel setBackgroundColor:RGBA(102, 203, 255, .3)];
        [detailLabel setLayerWidth:0.6f layerColor:COLOR_MAIN radius:2.0f];
        if (i == arr.count - 1) {
...
-(void)addLeftViews{
    ...
    for (int i = 0;i< _dataArrOfY.count ;i++ ) {
        ...
        UILabel *leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, i*(Ymargin)-Ymargin/2, titleWOfY, Ymargin+10)];
...
//    _titleOfY.textColor = [UIColor grayColor];
...
//    _titleOfX = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.scrollBgView1.frame), CGRectGetMaxY(self.chartScrollView.frame) - 40, 30, 20)];
