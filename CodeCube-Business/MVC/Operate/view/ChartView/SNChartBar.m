...
static const NSInteger kYEqualPaths = 6;//y轴为5等份
static const CGFloat kTopSpace = 10.f;//距离顶部y值
static const CGFloat kBarWidth = 20.f;
...
//    _shapeLayer.cornerRadius = 3;
//    _shapeLayer.masksToBounds = YES;
    _shapeLayer.fillColor = [UIColor clearColor].CGColor;
    _shapeLayer.strokeEnd = 0.f;
    _shapeLayer.strokeColor = COLOR_MAIN.CGColor;
...
- (void)addXLabel:(CGPoint)point andIndex:(NSInteger)index {
    ...
    label.center = CGPointMake(point.x + kBarWidth/2 + 10, chartBarTheYAxisSpan * kYEqualPaths + kTopSpace + 10);
...
- (void)addYLabel {
    for (NSInteger i = 0; i <= kYEqualPaths; i++) {
        ...
        if (i == kYEqualPaths) {
            ...
            label.text = [NSString stringWithFormat:@"%.0f",_yMax - _yMax/6 * i];
        }
    }
}
//柱状图标题
- (void)addBatTitle:(CGPoint)point andIndex:(NSInteger)index {
    ...
    label.center = CGPointMake(point.x, point.y - 5.f);
